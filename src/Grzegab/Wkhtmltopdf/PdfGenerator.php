...
class PdfGenerator extends AppController
{
    ...
    public function generatePDF()
    {
        ...
        if($this->pdfDownload) {
            $this->response->withType('pdf');
            $this->response->withCharset($this->encoding);
            $this->response->withFile($this->pdfTmpFile, ['download' => true, 'name' => $this->pdfName . '.pdf']);
        }
    ...
    private function renderHtml()
    {
        ...
        if(!empty($this->creatorClass->viewBuilder()->getTemplate())) {
            $viewFile = $this->creatorClass->viewBuilder()->getTemplate();
        } else {
            throw new \Exception('Please set template file');
        }
        return $this->creatorClass->render($viewFile, $layout)->getBody();
    }
